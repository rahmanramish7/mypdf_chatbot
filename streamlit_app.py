...
RETRIEVE_K_DEFAULT = int(os.getenv("RETRIEVE_K", 20))

# --- utils ---
def ensure_dir(p: Path):
    # If a FILE exists where a folder should be, delete it; then make dirs.
...
def get_embeddings():
    """Loads HuggingFace sentence-transformers embeddings once per container."""
    return HuggingFaceEmbeddings(model_name=EMBEDDING_MODEL)

@st.cache_resource(show_spinner=False)
def get_llm():
    """Creates the Groq Chat model once per container."""
    if not GROQ_API_KEY:
        raise RuntimeError(
            "Set GROQ_API_KEY in the environment (e.g., in Render env vars)."
        )
    return ChatGroq(model=GROQ_MODEL, temperature=0)

@st.cache_resource(show_spinner=False)
def load_index_if_exists(index_dir_str: str):
    """
    Loads a FAISS index (if present) once per container.
    Returns None if no index on disk.
    """
    p = Path(index_dir_str)
    if p.exists() and p.is_dir():
        return FAISS.load_local(
            index_dir_str, get_embeddings(), allow_dangerous_deserialization=True
        )
    return None
...
# Build or update the FAISS index on disk, always reusing cached embeddings
...
with st.sidebar:
    ...
    retrieve_k = st.slider("Retrieve-k", 5, 50, RETRIEVE_K_DEFAULT)
    top_k = st.slider("Top-k", 2, 12, TOP_K_DEFAULT)
    ...
    if st.button("Reset Index"):
        ...
        if INDEX_DIR.exists():
            if INDEX_DIR.is_dir():
                ...
                INDEX_DIR.unlink()
        # Optional: clear cached loaded index so a fresh one is loaded next time
        load_index_if_exists.clear()
...
if "vs" not in st.session_state:
    # Try to load an existing index lazily (does nothing if none)
    st.session_state.vs = load_index_if_exists(str(INDEX_DIR))
...
if uploads:
    ...
    with st.spinner("Building/merging FAISS index..."):
        st.session_state.vs = build_or_update_index(chunks)
...
# LLM is created lazily & cached; show a neat error in the sidebar if key missing
try:
    llm = get_llm()
        ...
        if st.session_state.vs is None:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking…"):
                    docs = st.session_state.vs.similarity_search(user_q, k=retrieve_k)
                    ctx_blocks = []
